...
import { CheckCircle2, AlertCircle, Filter, Loader, Clock, Thermometer, ChevronLeft, ChevronRight, Download, Bell } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import { formatters } from '../utils/formatters';
import { useStore } from '../store/useStore';
import { THEME_COLORS } from '../config/theme';
import AlertHeatmap from '../components/AlertHeatmap';
import { DEFAULT_BUILDING_CONFIG } from '../services/simulationEngine';
...
function AlertsPage() {
  ...
  // Severity config — fully adaptive for both modes
    ...
    <div className="section">
        ...
        <div className={`p-6 rounded-2xl border flex items-start justify-between ${
          ...
          <div>
            ...
            <p className={`ml-[3.25rem] text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>
              {pagination.total} total alerts &mdash; Page {pagination.page} of {pagination.pages}
            </p>
          </div>
      ...
      {resolveError && (
        <div className={`mb-6 p-4 rounded-xl border flex gap-3 ${
          ...
          <div className="flex-1">
            <p className={`text-sm font-medium ${isDark ? 'text-yellow-200' : 'text-yellow-800'}`}>Action failed</p>
            <p className={`text-xs mt-1 ${isDark ? 'text-yellow-300' : 'text-yellow-700'}`}>{resolveError}</p>
          </div>
        </div>
      )}
      ...
      <div className={`mb-8 p-6 rounded-2xl border transition-colors ${
        ...
        <AlertHeatmap
          alerts={alerts}
          rooms={DEFAULT_BUILDING_CONFIG.buildings.flatMap(b => b.rooms)}
          buildingId="block-a"
        />
      </div>
          ...
          {filteredAlerts.length > 0 ? (
                  ...
                  <div className="flex-shrink-0">
                    {alert.status === 'ACTIVE' && (
                      <motion.button
                        ...
                        className={`px-4 py-2 rounded-xl font-medium text-sm flex items-center gap-2 transition-all disabled:opacity-50 disabled:cursor-not-allowed border ${
                          isDark
            ...
            <motion.div
              ...
              className={`text-center py-20 rounded-2xl border ${
                isDark
                  ? 'bg-gradient-to-br from-blue-900/10 to-cyan-900/10 border-slate-700/40'
                  : 'bg-gradient-to-br from-blue-50 to-cyan-50 border-blue-200'
              }`}
            >
              <motion.div
      ...
      {pagination.pages > 1 && (
        ...
        >
          <motion.button
            ...
            disabled={page === 1}
            className={`px-4 py-2 rounded-xl font-medium flex items-center gap-2 border transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
              isDark
